...
parser.add_argument('--model_path', dest='model_path', default='checkpoints/001_checkpoint.pth.tar',
...
def main():
    ...
    for line in f:
        ...
        bs, n , h, w = image.size()
        result_var = torch.autograd.Variable(torch.zeros((bs, 1, h, w)).float().cuda())
        x_sem = model(image, result_var)
        x_sem = torch.nn.functional.softmax(x_sem,dim=1)[:,1,:,:].unsqueeze(1).contiguous()
        x_sem1 = model(image, x_sem)
        x_sem1 = torch.nn.functional.softmax(x_sem1,dim=1)[:,1,:,:].unsqueeze(1).contiguous()
        output = model(image, x_sem1)
