...
best_prec = 0
vgg_dict = {'features.0.weight': 'features.0.weight',
    'features.0.bias':'features.0.bias',
    'features.2.weight':'features.2.weight',
    'features.2.bias':'features.2.bias',
    'features1.2.weight':'features.5.weight',
    'features1.2.bias':'features.5.bias',
    'features1.4.weight':'features.7.weight',
    'features1.4.bias':'features.7.bias',
    'features2.2.weight':'features.10.weight',
    'features2.2.bias':'features.10.bias',
    'features2.4.weight':'features.12.weight',
    'features2.4.bias':'features.12.bias',
    'features2.6.weight':'features.14.weight',
    'features2.6.bias':'features.14.bias',
    'features2.8.weight':'features.16.weight',
    'features2.8.bias':'features.16.bias'}
reverse_dict = {v: k for k, v in vgg_dict.items()}
...
def main():
    ...
    model_vgg = lanenet.VGG_base()
    params = torch.load('/mnt/lustre/dingmingyu/.torch/models/vgg19-dcbb9e9d.pth')
    pretrained_dict = {reverse_dict[k]: v for k, v in params.items() if k in reverse_dict and reverse_dict[k] in model_vgg.state_dict()}
...
def train(train_loader, model, model_vgg, criterion, criterion_mse, optimizer, epoch):
    ...
    for i, (input, target_ins, n_objects) in enumerate(train_loader):
       ...
       # n_objects_normalized_var = torch.autograd.Variable(n_objects_normalized)
        ...
        n ,x ,y = x_prob.size()
        x_vgg, x_vgg1, x_vgg2 = model_vgg(x_prob.unsqueeze(1).expand(n, 3, x, y))
        y_vgg, y_vgg1, y_vgg2 = model_vgg(target_var.unsqueeze(1).expand(n, 3, x, y).float())
        y_vgg_nograd = y_vgg.detach()
        y_vgg_nograd1 = y_vgg1.detach()
        y_vgg_nograd2 = y_vgg2.detach()
        
        loss_vgg = criterion_mse(x_vgg, y_vgg_nograd) + criterion_mse(x_vgg1, y_vgg_nograd1) + criterion_mse(x_vgg2, y_vgg_nograd2)
